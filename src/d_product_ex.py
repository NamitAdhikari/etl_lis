...
import logging
logging.basicConfig(filename=f'{BASE_DIR}/log/d_product_ex.log',
...
try:
    ...
    logging.info("Cursor Execution - Fetched all Product Data")

    product_df = cur.fetch_pandas_all()
    logging.info("Converted fetched Product data to Pandas Dataframe")
    product_df.to_csv(f'{BASE_DIR}/data/d_product_data.csv', index=False)
    logging.info("Converted Dataframe to csv, and saved to 'd_product_data.csv' file")
